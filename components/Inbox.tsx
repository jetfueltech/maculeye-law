        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-stone-100 flex justify-between items-center bg-stone-50">
              ...
              <button onClick={() => { setIsLinkModalOpen(false); setCaseSearchTerm(''); }} className="text-stone-400 hover:text-stone-600">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <div className="p-6">
              <p className="text-sm text-stone-600 mb-3">Select a case to tag <strong>{selectedThread?.subject}</strong></p>
              {cases.length > 3 && (
                <div className="relative mb-3">
                  <input
                    type="text"
                    placeholder="Search cases..."
                    className="w-full pl-8 pr-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                    value={caseSearchTerm}
                    onChange={(e) => setCaseSearchTerm(e.target.value)}
                    autoFocus
                  />
                  <svg className="w-3.5 h-3.5 text-stone-400 absolute left-2.5 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                </div>
              )}
              <div className="space-y-2 max-h-72 overflow-y-auto">
                {cases.length === 0 ? (
                  <div className="text-center py-6 text-stone-400">
                    <svg className="w-8 h-8 mx-auto mb-2 text-stone-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" /></svg>
                    <p className="text-sm font-medium">No cases found</p>
                    <p className="text-xs mt-1">Create a case first, then tag emails to it</p>
                  </div>
                ) : (
                  cases
                    .filter(c => {
                      ...
                      return (
                        ...
                        getCaseTag(c.id).toLowerCase().includes(q)
                      );
                    })
                    .map(c => (
                      <button
                        key={c.id}
                        onClick={() => { handleConfirmLink(c.id); setCaseSearchTerm(''); }}
                      ...
                      >
                        <div className="min-w-0">
                          <p className="font-bold text-stone-900 group-hover:text-blue-700 truncate">{getCaseTag(c.id)}</p>
                          <p className="text-xs text-stone-500 truncate">
                            {c.caseNumber ? `#${c.caseNumber}` : c.id} - {c.status.replace(/_/g, ' ')}
                          </p>
                        </div>
