...
export const MemberPicker: React.FC<MemberPickerProps> = ({ value, onChange, compact, placeholder }) => {
      ...
      <div className="relative" ref={ref}>
        ...
        >
          {value ? (
            ...
            <span className="w-6 h-6 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 group-hover:border-blue-400 group-hover:text-blue-500 transition-colors">
        ...
        {open && (
          <div className="absolute top-8 left-0 z-50 bg-white border border-slate-200 shadow-xl rounded-xl w-56 py-1 animate-fade-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="max-h-48 overflow-y-auto">
              ...
              {filtered.length === 0 && (
                <div className="px-3 py-4 text-xs text-slate-400 text-center">No members found</div>
              )}
              {filtered.map(m => (
                <button
                  key={m.id}
                  onClick={() => { onChange({ id: m.id, name: m.name, initials: m.initials }); setOpen(false); setSearch(''); }}
                  className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center gap-2 ${value?.id === m.id ? 'bg-blue-50 text-blue-700' : 'text-slate-700'}`}
                >
                  ...
                  <span className="text-[10px] text-slate-400 ml-auto flex-shrink-0">{m.role}</span>
                  {value?.id === m.id && (
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={(e) => { e.stopPropagation(); setOpen(!open); }}
        className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg hover:border-blue-300 transition-colors text-sm group w-full"
      >
        {value ? (
          <>
            ...
            <span className="text-slate-800 font-medium truncate">{value.name}</span>
          </>
        ) : (
          <>
            <span className="w-6 h-6 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 group-hover:border-blue-400 group-hover:text-blue-500 transition-colors flex-shrink-0">
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
            </span>
            <span className="text-slate-400 text-xs">{placeholder || 'Assign member'}</span>
          </>
        )}
        <svg className="w-3 h-3 ml-auto text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
      </button>

      {open && (
        <div className="absolute top-10 left-0 z-50 bg-white border border-slate-200 shadow-xl rounded-xl w-64 py-1 animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className="px-3 py-2">
            <input
              type="text"
              placeholder="Search members..."
          ...
          <div className="max-h-56 overflow-y-auto">
            ...
            {filtered.length === 0 && (
              <div className="px-3 py-6 text-sm text-slate-400 text-center">No members found</div>
            )}
            {filtered.map(m => (
              <button
                key={m.id}
                onClick={() => { onChange({ id: m.id, name: m.name, initials: m.initials }); setOpen(false); setSearch(''); }}
                className={`w-full text-left px-3 py-2.5 text-sm hover:bg-slate-50 flex items-center gap-2.5 ${value?.id === m.id ? 'bg-blue-50 text-blue-700' : 'text-slate-700'}`}
              >
