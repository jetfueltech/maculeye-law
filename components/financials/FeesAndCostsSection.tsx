  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-stone-100">
        <h3 className="text-sm font-bold text-stone-800 uppercase tracking-wide">Fees, Costs, Financial Liens, and Expenses</h3>
      </div>
      <div className="p-6 space-y-6">
        <div>
          <p className="text-xs font-bold text-stone-700 mb-3">1. Attorney Fees</p>
          {editing ? (
            <div className="flex items-end gap-4 max-w-md">
              <div className="flex-1">
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-3 top-1/2 -transtone-y-1/2 text-stone-400 text-sm">%</span>
                </div>
              </div>
              <div className="pb-2 text-sm text-stone-500">
                per contract: <span className="font-semibold text-stone-800">{currency(grossSettlement * ((form.feePercentage || 0) / 100))}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-6 pl-4">
              <span className="text-sm text-stone-600">{pct}% per contract:</span>
              <span className="text-sm font-semibold text-stone-800 tabular-nums">{currency(attorneyFee)}</span>
            </div>
          )}
        </div>

        <div className="border-t border-stone-100 pt-4">
          <p className="text-xs font-bold text-stone-700 mb-3">2. Miscellaneous Costs</p>
          {editing ? (
            ...
            <div className="space-y-1.5 pl-4">
              ...
              <div className="border-t border-stone-100 pt-2 mt-2 flex justify-between items-center">
                <span className="text-sm font-semibold text-stone-700">Total Miscellaneous Costs</span>
                <span className="text-sm font-semibold text-stone-800 tabular-nums">{currency(totalMiscCosts)}</span>
              </div>
            </div>
          )}
        </div>
...
function CostRow({ letter, label, value }: { letter: string; label: string; value: string }) {
  return (
    <div className="flex justify-between items-center py-0.5">
      <span className="text-sm text-stone-600">{letter}. {label}</span>
...
function DollarInput({ value, onChange }: { value: number | undefined; onChange: (v: number) => void }) {
  return (
    <div className="relative">
      ...
      <input
        type="text"
        className="w-full px-3 py-2 pl-7 bg-white border border-stone-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow"
        placeholder="0.00"
        value={value ?? ''}
        onChange={e => {
