...
const thClass = "text-left text-[11px] font-semibold text-stone-400 uppercase tracking-wider px-3 py-2";
    ...
    <div className="space-y-6">
            ...
            {unlinkedProviders.map(p => (
              <div key={p.id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-blue-100">
                <div>
                  ...
                  <span className="ml-2 text-xs text-stone-500">({currency(p.totalCost)})</span>
                </div>
                <button
      ...
      <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-stone-100">
          <h3 className="text-sm font-bold text-stone-800 uppercase tracking-wide">5. Medical Expenses</h3>
        </div>
          ...
          <table className="w-full min-w-[700px]">
            ...
            <tbody className="divide-y divide-stone-50">
              {expenses.map((exp, i) => {
                    ...
                    <tr key={exp.id} className="bg-blue-50/50">
                      ...
                      <td className="pr-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={cancelEditExpense} className="p-1 text-stone-400 hover:text-stone-600 transition-colors" title="Cancel">
                ...
                return (
                  <tr key={exp.id} className="hover:bg-stone-50/50 group">
                    ...
                    <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(exp.totalCharges)}</td>
                    <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(exp.amountDue)}</td>
                    <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(exp.reductionAmount)}</td>
                    <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(exp.clientResponsible)}</td>
                    <td className="px-3 py-2 text-xs text-stone-500 italic">{exp.notes || ''}</td>
                    <td className="pr-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => startEditExpense(exp)} className="p-1 text-stone-400 hover:text-blue-600 transition-colors" title="Edit">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </button>
                        <button onClick={() => removeExpense(exp.id)} className="p-1 text-stone-400 hover:text-rose-500 transition-colors" title="Remove">
              ...
              {expenses.length === 0 && (
                <tr><td colSpan={7} className="px-6 py-4 text-center text-sm text-stone-400">No medical expenses recorded</td></tr>
              )}
              {expenses.length > 0 && (
                <tr className="bg-stone-50 font-semibold">
                  <td className="px-3 py-2 text-sm text-stone-800">Totals</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(totals.charges)}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(totals.due)}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(totals.reduction)}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(totals.clientResp)}</td>
                  <td colSpan={2}></td>
                </tr>
              )}
            </tbody>
        ...
        <div className="px-4 py-3 border-t border-stone-100 bg-stone-50/50">
          <div className="flex items-center gap-2">
            ...
            <button onClick={addExpense} className="px-3 py-2 bg-black text-white text-sm font-semibold rounded-lg hover:bg-stone-800 shrink-0">Add</button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-stone-100">
          <h3 className="text-sm font-bold text-stone-800 uppercase tracking-wide">6. Health Insurance Subrogation</h3>
        </div>
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-stone-50">
              {subs.map((sub, i) => (
                <tr key={sub.id} className="hover:bg-stone-50/50">
                  <td className="px-3 py-2 text-sm text-stone-700">{String.fromCharCode(65 + i)}. {sub.carrier}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(sub.originalBill)}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(sub.compromisedBill)}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(sub.reductionAmount)}</td>
                  <td className="px-3 py-2 text-xs text-stone-500 italic">{sub.notes || ''}</td>
                  <td className="pr-3">
                    <button onClick={() => removeSub(sub.id)} className="p-1 text-stone-400 hover:text-rose-500 transition-colors">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                  </td>
                </tr>
              ...
              {subs.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-4 text-center text-sm text-stone-400">No subrogation claims</td></tr>
              )}
              {subs.length > 0 && (
                <tr className="bg-stone-50 font-semibold">
                  <td className="px-3 py-2 text-sm text-stone-800">Totals</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(subTotals.original)}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(subTotals.compromised)}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums text-right">{currency(subTotals.reduction)}</td>
                  <td colSpan={2}></td>
                </tr>
              )}
            </tbody>
        ...
        <div className="px-4 py-3 border-t border-stone-100 bg-stone-50/50">
          <div className="flex items-center gap-2">
            ...
            <button onClick={addSub} className="px-3 py-2 bg-black text-white text-sm font-semibold rounded-lg hover:bg-stone-800 shrink-0">Add</button>
          </div>
        </div>
      </div>
    </div>
