    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-stone-100 flex justify-between items-center">
          <h3 className="text-sm font-bold text-stone-800 uppercase tracking-wide">3. Financial Liens</h3>
          <span className="text-xs font-semibold text-stone-500">Total: {currency(totalLiens)} | Reduced: {currency(totalLiensReduced)}</span>
        </div>
        <div>
          <table className="w-full">
            ...
            <tbody className="divide-y divide-stone-50">
              {liens.map((lien, i) => (
                <tr key={lien.id} className="hover:bg-stone-50/50">
                  <td className="px-3 py-2 text-sm text-stone-700">{String.fromCharCode(65 + i)}. {lien.description || '--'}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums">{currency(lien.amount)}</td>
                  <td className="px-3 py-2 text-sm text-stone-500">{lien.date || '--'}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums">{currency(lien.reducedAmount)}</td>
                  <td className="px-3 py-2 text-sm text-stone-500">{lien.checkNumber || '--'}</td>
                  <td className="pr-3">
                    <button onClick={() => removeLien(lien.id)} className="p-1 text-stone-400 hover:text-rose-500 transition-colors">
              ...
              {liens.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-4 text-center text-sm text-stone-400">No financial liens</td></tr>
              )}
            </tbody>
          </table>
          <div className="px-4 py-3 border-t border-stone-100 bg-stone-50/50">
            <div className="flex items-center gap-2">
              ...
              <button onClick={addLien} className="px-3 py-2 bg-black text-white text-sm font-semibold rounded-lg hover:bg-stone-800 shrink-0">Add</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-stone-100 flex justify-between items-center">
          <h3 className="text-sm font-bold text-stone-800 uppercase tracking-wide">4. Third Party Loans</h3>
          <span className="text-xs font-semibold text-stone-500">Total: {currency(totalLoans)} | Final: {currency(totalLoansFinal)}</span>
        </div>
        <div>
          <table className="w-full">
            <thead>
            ...
            <tbody className="divide-y divide-stone-50">
              {loans.map((loan, i) => (
                <tr key={loan.id} className="hover:bg-stone-50/50">
                  <td className="px-3 py-2 text-sm text-stone-700">{String.fromCharCode(65 + i)}. {loan.description || '--'}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums">{currency(loan.loanAmount)}</td>
                  <td className="px-3 py-2 text-sm text-stone-500">{loan.loanDate || '--'}</td>
                  <td className="px-3 py-2 text-sm text-stone-800 tabular-nums">{currency(loan.finalAmount)}</td>
                  <td className="px-3 py-2 text-sm text-stone-500">{loan.dateDue || '--'}</td>
                  <td className="pr-3">
                    <button onClick={() => removeLoan(loan.id)} className="p-1 text-stone-400 hover:text-rose-500 transition-colors">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                  </td>
                </tr>
              ...
              {loans.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-4 text-center text-sm text-stone-400">No third party loans</td></tr>
              )}
            </tbody>
          </table>
          <div className="px-4 py-3 border-t border-stone-100 bg-stone-50/50">
            <div className="flex items-center gap-2">
              ...
              <button onClick={addLoan} className="px-3 py-2 bg-black text-white text-sm font-semibold rounded-lg hover:bg-stone-800 shrink-0">Add</button>
            </div>
          </div>
        </div>
      </div>
