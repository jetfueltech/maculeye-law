      ...
      <div className="px-6 py-5 border-b border-stone-100">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-1">{completedCount} of {checklist.length} requirements met</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className={`text-sm font-bold ${progress === 100 ? 'text-emerald-600' : 'text-stone-600'}`}>{progress}%</span>
          </div>
        </div>
      </div>
          ...
          <div key={idx} className={`px-6 py-4 flex items-center gap-4 ${item.complete ? '' : item.critical ? 'bg-rose-50/30' : ''}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
              item.complete
                ...
                : 'bg-stone-100 text-stone-300 border-2 border-stone-200'
            }`}>
              {item.complete ? (
            ...
            <div className="flex-1">
              <span className={`text-sm font-medium ${item.complete ? 'text-stone-500 line-through' : 'text-stone-900'}`}>
                {item.label}
              </span>
              {item.detail && (
