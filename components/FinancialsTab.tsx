    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
          <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Demand Letter</h3>
        </div>
        <div className="p-6">
          {editing ? (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Demand Amount</label>
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Demand Notes</label>
                <input
                  type="text"
                  className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"
      ...
      <div className="flex justify-end">
        {!editing ? (
          <button onClick={startEdit} className="px-4 py-2 bg-slate-800 text-white text-sm font-semibold rounded-lg hover:bg-slate-700 transition-colors flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            Edit Settlement & Fees
          </button>
        ) : (
          <div className="flex gap-2">
            <button onClick={cancel} className="px-4 py-2 text-sm text-slate-600 hover:text-slate-800 font-medium border border-slate-200 rounded-lg">Cancel</button>
            <button onClick={save} className="px-4 py-2 bg-slate-800 text-white text-sm font-semibold rounded-lg hover:bg-slate-700 transition-colors">Save Changes</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-slate-100 rounded-2xl border border-slate-200 px-6 py-4 flex justify-between items-center">
        <span className="text-sm font-bold text-slate-700 uppercase">Total Fees, Costs, Financial Liens, and Expenses</span>
        <span className="text-lg font-bold text-slate-900 tabular-nums">{currency(d.totalDeductions)}</span>
      </div>
