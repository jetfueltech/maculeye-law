...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className={`${isCollapsed ? 'w-20' : 'w-64'} bg-slate-900 text-white h-screen flex flex-col fixed left-0 top-0 border-r border-slate-800 shadow-xl z-20 transition-all duration-300`}>
      ...
      {firms.length > 0 && (
        <div className={`px-4 py-3 border-b border-slate-800 relative ${isCollapsed ? 'flex justify-center' : ''}`}>
          {isCollapsed ? (
            <div
              className="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-300 cursor-pointer hover:bg-slate-600 transition-colors"
            ...
            <>
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 transition-colors group"
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="w-6 h-6 rounded-md bg-blue-500 flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0">
                    {activeFirm?.name.slice(0, 2).toUpperCase() || '--'}
                  </div>
                  <div className="min-w-0 text-left">
                    <p className="text-xs font-bold text-white truncate">{activeFirm?.name || 'Select Firm'}</p>
                    {activeFirmRole && (
                      <p className="text-[10px] text-slate-400">{ROLE_LABELS[activeFirmRole] || activeFirmRole}</p>
                    )}
                  </div>
                </div>
                {firms.length > 1 && (
              ...
              {showFirmMenu && firms.length > 1 && (
                <div className="absolute left-4 right-4 top-full mt-1 bg-slate-800 border border-slate-700 rounded-xl shadow-xl z-30 overflow-hidden animate-fade-in">
                  <div className="px-3 py-2 border-b border-slate-700">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Switch Firm</p>
                  </div>
                  {firms.map(firm => (
                    <button
                      key={firm.id}
                      onClick={() => { switchFirm(firm.id); setShowFirmMenu(false); }}
                      className={`w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-slate-700 transition-colors text-left ${activeFirm?.id === firm.id ? 'bg-slate-700' : ''}`}
                    >
                      <div className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${activeFirm?.id === firm.id ? 'bg-blue-500 text-white' : 'bg-slate-600 text-slate-300'}`}>
                        {firm.name.slice(0, 2).toUpperCase()}
                      </div>
                      <span className="text-sm font-medium text-slate-200 truncate">{firm.name}</span>
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            {(item.id === 'dashboard' || item.id === 'inbox' || item.id === 'tasks') && isCollapsed && (
              <div className={`absolute top-1 right-1 w-2 h-2 rounded-full ${item.id === 'inbox' ? 'bg-red-500' : item.id === 'tasks' ? 'bg-rose-500' : 'bg-blue-500'}`} />
            )}
          </button>
        ))}
      </nav>
        ...
        <div className={`flex items-center ${isCollapsed ? 'flex-col gap-2' : 'gap-3'}`}>
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-white truncate">{profile?.full_name || profile?.username || profile?.email || '...'}</p>
              <p className="text-xs text-slate-500">{systemRoleLabel}</p>
            </div>
          )}
          <button
            onClick={signOut}
