...
const JURISDICTION_COLORS: Record<string, string> = {
  ...
  federal: 'bg-stone-100 text-stone-700 border-stone-300',
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <svg className="w-4 h-4 absolute left-3 top-1/2 -transtone-y-1/2 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showForm && (
        <div className="bg-white border border-stone-200 rounded-2xl p-6 space-y-5 animate-fade-in">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-stone-800">{editingId ? 'Edit Police Department' : 'New Police Department'}</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Department Name</label>
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Jurisdiction</label>
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Website</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Records Division Phone</label>
          ...
          <div>
            <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Records Email</label>
            ...
            {filtered.map(d => (
              <div key={d.id}>
                ...
                >
                  <div className="flex items-center gap-4 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-sm text-stone-900">{d.name}</span>
                  ...
                  <svg className={`w-4 h-4 text-stone-400 transition-transform ${expandedId === d.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {expandedId === d.id && (
                  <div className="px-6 pb-4 bg-stone-50/50 border-t border-stone-100 animate-fade-in">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 py-4">
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Main Phone</span><span className="text-sm text-stone-800">{d.phone || '--'}</span></div>
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Records Phone</span><span className="text-sm text-stone-800">{d.records_phone || '--'}</span></div>
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Fax</span><span className="text-sm text-stone-800">{d.fax || '--'}</span></div>
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Records Email</span><span className="text-sm text-stone-800">{d.records_email || '--'}</span></div>
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Address</span><span className="text-sm text-stone-800">{[d.address, d.city, d.state, d.zip].filter(Boolean).join(', ') || '--'}</span></div>
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Mailing Address</span><span className="text-sm text-stone-800">{[d.mailing_address, d.mailing_city, d.mailing_state, d.mailing_zip].filter(Boolean).join(', ') || '--'}</span></div>
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Website</span><span className="text-sm text-stone-800">{d.website || '--'}</span></div>
                      <div className="md:col-span-2"><span className="text-[11px] font-bold text-stone-400 uppercase block">Notes</span><span className="text-sm text-stone-800">{d.notes || '--'}</span></div>
                    </div>
                    <div className="flex justify-end gap-2 pt-2 border-t border-stone-100">
                      <button onClick={() => handleEdit(d)} className="px-3 py-1.5 text-xs font-medium text-stone-700 bg-white border border-stone-200 rounded-lg hover:bg-stone-50 transition-colors">Edit</button>
