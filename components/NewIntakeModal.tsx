...
export const NewIntakeModal: React.FC<NewIntakeModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const inputClass = "w-full bg-white border border-stone-300 rounded-lg px-3 py-2 text-stone-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-stone-400 transition-all shadow-sm disabled:bg-stone-100 disabled:text-stone-400";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="bg-stone-50 px-6 py-4 border-b border-stone-100 flex justify-between items-center flex-shrink-0">
          <div>
            <h3 className="text-xl font-bold text-stone-800">Agency Intake Submission</h3>
            <p className="text-sm text-stone-500">New Client Webhook Simulation</p>
          </div>
        ...
        <div className="px-6 pt-6 flex-shrink-0">
          <div className="flex items-center justify-between relative">
            <div className="absolute left-0 top-1/2 -transtone-y-1/2 w-full h-0.5 bg-stone-100 -z-10"></div>
            {steps.map((label, i) => (
              <div key={i} className="flex flex-col items-center cursor-pointer group" onClick={() => setStep(i + 1)}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all border-2 ${step > i ? 'bg-green-500 border-green-500 text-white' : step === i + 1 ? 'bg-blue-600 border-blue-600 text-white ring-4 ring-blue-50' : 'bg-white border-stone-200 text-stone-400 group-hover:border-stone-300'}`}>
                  {step > i ? '✓' : i + 1}
                </div>
                <span className={`text-xs mt-2 font-medium ${step === i + 1 ? 'text-blue-600' : 'text-stone-500'}`}>{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {step === 1 && (
            <div className="space-y-4 animate-fade-in">
              <h4 className="font-semibold text-stone-800 border-b border-stone-100 pb-2 mb-4">Client Demographics</h4>
          ...
          {step === 2 && (
            <div className="space-y-4 animate-fade-in">
              <h4 className="font-semibold text-stone-800 border-b border-stone-100 pb-2 mb-4">Incident & Vehicle</h4>
              <div className="grid grid-cols-2 gap-5">
                <div>
              ...
              <div className="bg-stone-50 p-4 rounded-xl border border-stone-100 mt-2">
                <h5 className="font-semibold text-stone-700 mb-3 text-sm">Client Vehicle Information</h5>
          ...
          {step === 3 && (
            <div className="space-y-8 animate-fade-in">
              ...
              {/* Client */}
              <div>
                <h4 className="font-semibold text-stone-800 border-b border-stone-100 pb-2 mb-4">Client Insurance</h4>
                <div className="grid grid-cols-12 gap-5">
                  <div className="col-span-6">
               ...
               {/* Other */}
               <div>
                <h4 className="font-semibold text-stone-800 border-b border-stone-100 pb-2 mb-4">Other Party Insurance</h4>
                <div className="grid grid-cols-12 gap-5">
          ...
          {step === 4 && (
             <div className="space-y-4 animate-fade-in">
               <h4 className="font-semibold text-stone-800 border-b border-stone-100 pb-2 mb-4">Medical Treatment</h4>
               <div>
          ...
          {step === 5 && (
            <div className="space-y-4 animate-fade-in">
              <h4 className="font-semibold text-stone-800 border-b border-stone-100 pb-2 mb-4">Key Documents</h4>
              <p className="text-sm text-stone-600 mb-4 bg-blue-50 p-3 rounded-lg border border-blue-100 text-blue-800">Please simulate uploading the following key items required for valid intake processing. (Images & PDF supported)</p>
              ...
              {documents.length > 0 && (
                <div className="mt-6">
                   <h5 className="text-xs font-bold uppercase text-stone-500 mb-3 tracking-wider">Attached Files ({documents.length})</h5>
                   <div className="space-y-2">
                     {documents.map((doc, i) => (
                       <div key={i} className="flex justify-between items-center bg-white p-3 rounded-lg border border-stone-200 shadow-sm text-sm hover:border-blue-300 transition-colors">
                          <div className="flex items-center space-x-3 overflow-hidden">
                             ...
                             <div className="truncate">
                               ...
                               <p className="text-xs text-stone-500">{doc.type} • {doc.mimeType?.includes('pdf') ? 'PDF' : 'Image'}</p>
                             </div>
                          </div>
                          <button onClick={() => removeDocument(i)} className="text-stone-400 hover:text-red-500 p-2 transition-colors rounded-full hover:bg-red-50">
        ...
        {/* Footer Actions */}
        <div className="bg-stone-50 px-6 py-4 border-t border-stone-100 flex justify-between flex-shrink-0">
           {step > 1 ? (
             <button onClick={() => setStep(step - 1)} className="px-5 py-2 text-stone-600 font-medium hover:text-stone-900 transition-colors hover:bg-stone-200 rounded-lg">
               Back
             </button>
           ) : <div></div>}
