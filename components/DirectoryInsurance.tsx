    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-white border border-stone-200 rounded-2xl p-6 space-y-5 animate-fade-in">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-stone-800">{editingId ? 'Edit Insurance Company' : 'New Insurance Company'}</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Company Name</label>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-stone-800 mb-1">{search || filterType !== 'all' ? 'No matching companies' : 'No Insurance Companies Yet'}</h3>
          <p className="text-sm text-stone-500">{search || filterType !== 'all' ? 'Try adjusting your search or filter.' : 'Add insurance companies to build your directory.'}</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
          <div className="px-6 py-3 bg-stone-50 border-b border-stone-200">
            <span className="text-xs font-bold text-stone-400 uppercase tracking-wider">{filtered.length} compan{filtered.length !== 1 ? 'ies' : 'y'}</span>
          </div>
            ...
            {filtered.map(c => (
              <div key={c.id}>
                ...
                >
                  <div className="flex items-center gap-4 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-sm text-stone-900">{c.name}</span>
                  ...
                  <svg className={`w-4 h-4 text-stone-400 transition-transform ${expandedId === c.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                </div>
                {expandedId === c.id && (
                  <div className="px-6 pb-4 bg-stone-50/50 border-t border-stone-100 animate-fade-in">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 py-4">
                      ...
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Website</span><span className="text-sm text-stone-800">{c.website || '--'}</span></div>
                      <div><span className="text-[11px] font-bold text-stone-400 uppercase block">Notes</span><span className="text-sm text-stone-800">{c.notes || '--'}</span></div>
                    </div>
                    <div className="flex justify-end gap-2 pt-2 border-t border-stone-100">
                      <button onClick={() => handleEdit(c)} className="px-3 py-1.5 text-xs font-medium text-stone-700 bg-white border border-stone-200 rounded-lg hover:bg-stone-50 transition-colors">Edit</button>
