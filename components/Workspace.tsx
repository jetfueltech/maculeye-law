...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string; dot: string }> = {
  ...
  INTAKE_PAUSED: { label: 'Paused', bg: 'bg-slate-100', text: 'text-slate-600', dot: 'bg-slate-400' },
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <h3 className="text-base font-bold text-slate-900 truncate">{caseFile.clientName}</h3>
            <p className="text-xs text-slate-500 mt-0.5 truncate">DOL: {caseFile.accidentDate ? new Date(caseFile.accidentDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</p>
          </div>
          <div className="flex flex-col items-end gap-2 flex-shrink-0">
            {solDays !== null && (
              <div className={`text-center px-3 py-1.5 rounded-xl ${
                ...
                <p className={`text-xs font-bold ${solDays <= 30 ? 'text-red-700' : solDays <= 90 ? 'text-amber-700' : 'text-slate-600'}`}>
                  {solDays > 0 ? `${solDays}d` : 'EXPIRED'}
                </p>
                <p className={`text-[10px] ${solDays <= 30 ? 'text-red-500' : solDays <= 90 ? 'text-amber-500' : 'text-slate-400'}`}>SOL</p>
              </div>
            )}
            <button
        ...
        {currentStage && (
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-500 font-medium whitespace-nowrap">{currentStage.label}</span>
          </div>
        )}
      ...
      {expanded && openTasks.length > 0 && (
        <div className="border-t border-slate-100 bg-slate-50 px-5 py-3 space-y-2">
          <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Open Tasks</p>
            ...
            return (
              <div key={task.id} className="flex items-center gap-2.5 group">
                <button
                  ...
                  className="w-4 h-4 rounded border-2 border-slate-300 hover:border-blue-500 hover:bg-blue-50 transition-colors flex-shrink-0"
                  title="Mark complete"
                />
                <div className="flex-1 min-w-0">
          ...
          {openTasks.length > 6 && (
            <p className="text-[11px] text-slate-400 text-center pt-1">+{openTasks.length - 6} more tasks</p>
          )}
          <button
            onClick={(e) => { e.stopPropagation(); onSelectCase(caseFile); }}
...
const NewCaseRow: React.FC<{ caseFile: CaseFile; onSelectCase: (c: CaseFile) => void }> = ({ caseFile, onSelectCase }) => {
  ...
  return (
    <div className="flex items-center gap-4 py-3 px-4 hover:bg-slate-50 rounded-xl transition-colors group">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <p className="text-sm font-semibold text-slate-900 truncate">{caseFile.clientName}</p>
        ...
        <p className="text-xs text-slate-400 truncate">{caseFile.impact || caseFile.description?.slice(0, 60) || '—'}</p>
      </div>
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1">Here's your case workload for today.</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className={`rounded-2xl border p-5 ${overdueCount > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'}`}>
            <p className={`text-xs font-semibold uppercase tracking-wider mb-1 ${overdueCount > 0 ? 'text-red-500' : 'text-slate-500'}`}>Overdue Tasks</p>
            <p className={`text-3xl font-bold ${overdueCount > 0 ? 'text-red-700' : 'text-slate-900'}`}>{overdueCount}</p>
          </div>
          <div className={`rounded-2xl border p-5 ${dueTodayCount > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-200'}`}>
            <p className={`text-xs font-semibold uppercase tracking-wider mb-1 ${dueTodayCount > 0 ? 'text-amber-600' : 'text-slate-500'}`}>Due Today</p>
            <p className={`text-3xl font-bold ${dueTodayCount > 0 ? 'text-amber-700' : 'text-slate-900'}`}>{dueTodayCount}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 mb-8">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <div>
              <h2 className="text-sm font-bold text-slate-900">New Cases</h2>
              <p className="text-xs text-slate-400 mt-0.5">{newCases.length} case{newCases.length !== 1 ? 's' : ''} added</p>
            </div>
          ...
          <div className="px-1 py-2">
            {newCases.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-sm text-slate-400">No new cases in this period</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-50">
                ...
                {newCases.length > 8 && (
                  <p className="text-xs text-slate-400 text-center py-3">+{newCases.length - 8} more</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
              ...
              >
                ...
                {f.count !== undefined && f.count > 0 && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                    filter === f.id ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-600'
                  }`}>
                    {f.count}
                  </span>
                )}
