...
const BILL_STATUS_COLORS: Record<ERBillStatus, string> = {
  not_requested: 'bg-stone-100 text-stone-600 border-stone-200',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-stone-200 p-5">
          <div className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-1">ER Visits</div>
          <div className="text-2xl font-bold text-stone-900">{erVisits.length}</div>
        </div>
      ...
      {showAddForm ? (
        <div className="bg-white border border-stone-200 rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-bold text-stone-800">Add ER Visit</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase mb-1">ER Facility Name</label>
            ...
            <div>
              <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Visit Date</label>
              <input type="date" className={inputClass} value={newVisit.visitDate} onChange={e => setNewVisit({ ...newVisit, visitDate: e.target.value })} />
            </div>
          </div>
          <p className="text-xs text-stone-500">Three bill lines (Facility, Physician, Radiology) will be auto-created for tracking.</p>
          <div className="flex justify-end gap-3">
            <button onClick={() => setShowAddForm(false)} className="px-4 py-2 text-sm font-medium text-stone-600 hover:bg-stone-50 rounded-lg">Cancel</button>
        ...
        <button onClick={() => setShowAddForm(true)} className="w-full py-3 border-2 border-dashed border-stone-300 rounded-xl text-sm font-bold text-stone-500 hover:text-blue-600 hover:border-blue-300 hover:bg-blue-50/50 transition-all">
          + Add ER Visit
        </button>
      )}
      ...
      <div className="space-y-4">
        {erVisits.map(visit => {
          ...
          return (
            <div key={visit.id} className={`bg-white rounded-2xl border overflow-hidden transition-all ${overdue ? 'border-rose-200' : allDone ? 'border-emerald-200' : 'border-stone-200'}`}>
              <div
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="font-bold text-stone-900">{visit.facilityName}</h4>
                ...
                <svg className={`w-5 h-5 text-stone-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
              </div>

              {isExpanded && (
                <div className="border-t border-stone-100 p-6 space-y-6">
                  <div>
                    <h5 className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-3 pb-1 border-b border-stone-100">Medical Record</h5>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-stone-400 uppercase mb-1">Request Date</label>
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-3 pb-1 border-b border-stone-100">Bills (3-Bill Rule)</h5>
                    <div className="space-y-4">
                      {visit.bills.map(bill => {
                        ...
                        return (
                          <div key={bill.type} className={`rounded-xl border p-4 ${billOverdue ? 'border-rose-200 bg-rose-50/30' : 'border-stone-200'}`}>
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-stone-800">{BILL_TYPE_LABELS[bill.type]}</span>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              ...
                              <div>
                                <label className="block text-[10px] font-bold text-stone-400 uppercase mb-1">Amount ($)</label>
