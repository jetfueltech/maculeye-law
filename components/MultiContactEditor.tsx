...
const inputClass = "w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all";
const labelClass = "block text-[10px] font-bold text-slate-400 uppercase mb-0.5";
...
export const MultiContactEditor: React.FC<MultiContactEditorProps> = ({ directoryType, directoryId }) => {
  ...
  if (loading) return <div className="text-xs text-slate-400 py-2">Loading contacts...</div>;

  return (
    <div className="space-y-4 pt-3 border-t border-slate-100 mt-3">
      <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Additional Contact Information</div>

      {/* Phones */}
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[11px] font-bold text-slate-400 uppercase">Phone Numbers</span>
          <button onClick={() => setEditingPhone({ label: 'Main', phone_number: '' })} className={addBtnClass}>
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
            Add
          </button>
        </div>
        {phones.length === 0 && !editingPhone && <span className="text-xs text-slate-300">None added</span>}
        {phones.map(p => (
          <div key={p.id} className="flex items-center gap-2 py-1 group">
            <span className="text-[10px] font-semibold text-slate-400 uppercase w-16 flex-shrink-0">{p.label}</span>
            <span className="text-sm text-slate-800 flex-1">{p.phone_number}</span>
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => setEditingPhone({ id: p.id, label: p.label, phone_number: p.phone_number })} className="text-slate-300 hover:text-slate-600 p-0.5">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </button>
              <button onClick={async () => { await deletePhone(p.id); loadAll(); }} className="text-slate-300 hover:text-red-500 p-0.5">
        ...
        {editingPhone && (
          <div className="flex items-end gap-2 bg-slate-50 rounded-lg p-2 mt-1 animate-fade-in">
            <div className="w-24">
            ...
            <button onClick={() => setEditingPhone(null)} className="px-2 py-1.5 text-xs text-slate-500 hover:bg-slate-100 rounded-lg">Cancel</button>
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[11px] font-bold text-slate-400 uppercase">Fax Numbers</span>
          <button onClick={() => setEditingFax({ label: 'Main', fax_number: '' })} className={addBtnClass}>
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
            Add
          </button>
        </div>
        {faxes.length === 0 && !editingFax && <span className="text-xs text-slate-300">None added</span>}
        {faxes.map(f => (
          <div key={f.id} className="flex items-center gap-2 py-1 group">
            <span className="text-[10px] font-semibold text-slate-400 uppercase w-16 flex-shrink-0">{f.label}</span>
            <span className="text-sm text-slate-800 flex-1">{f.fax_number}</span>
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => setEditingFax({ id: f.id, label: f.label, fax_number: f.fax_number })} className="text-slate-300 hover:text-slate-600 p-0.5">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </button>
              <button onClick={async () => { await deleteFax(f.id); loadAll(); }} className="text-slate-300 hover:text-red-500 p-0.5">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
          </div>
        ...
        {editingFax && (
          <div className="flex items-end gap-2 bg-slate-50 rounded-lg p-2 mt-1 animate-fade-in">
            ...
            <button onClick={() => setEditingFax(null)} className="px-2 py-1.5 text-xs text-slate-500 hover:bg-slate-100 rounded-lg">Cancel</button>
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[11px] font-bold text-slate-400 uppercase">Addresses</span>
          <button onClick={() => setEditingAddr({ label: 'Main Office', address: '', city: '', state: '', zip: '' })} className={addBtnClass}>
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
            Add
          </button>
        </div>
        {addresses.length === 0 && !editingAddr && <span className="text-xs text-slate-300">None added</span>}
        {addresses.map(a => (
          <div key={a.id} className="flex items-center gap-2 py-1.5 group">
            <span className="text-[10px] font-semibold text-slate-400 uppercase w-16 flex-shrink-0">{a.label}</span>
            <span className="text-sm text-slate-800 flex-1">{[a.address, a.city, a.state, a.zip].filter(Boolean).join(', ')}</span>
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => setEditingAddr({ id: a.id, label: a.label, address: a.address, city: a.city, state: a.state, zip: a.zip })} className="text-slate-300 hover:text-slate-600 p-0.5">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </button>
              <button onClick={async () => { await deleteAddress(a.id); loadAll(); }} className="text-slate-300 hover:text-red-500 p-0.5">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
          </div>
        ...
        {editingAddr && (
          <div className="bg-slate-50 rounded-lg p-2 mt-1 space-y-2 animate-fade-in">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setEditingAddr(null)} className="px-2 py-1.5 text-xs text-slate-500 hover:bg-slate-100 rounded-lg">Cancel</button>
